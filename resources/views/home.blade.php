...
<div id="customSidebar">
  <div class="sidebar-header text-end">
    <button id="closeSidebarBtn" class="btn text-white fs-4">&times;</button>
  </div>
  ...
  <!-- Organization List generated by JS -->
  <ul class="sidebar-orgs" id="organizationList"></ul>
</div>
  </nav>
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const orgList = [
    {
      ...
      image: "{{ asset('images/ccms_logo.png') }}", // path to logo
      children: [
        { name: "ITS", image: "{{ asset('images/its_logo.png') }}", link: "{{ url('orgs/its_page') }}" },
        { name: "Praxis", image: "{{ asset('images/praxis_logo.png') }}", link: "{{ url('orgs/praxis_page') }}" }
      ]
    }
  ];
  ...
  orgList.forEach(org => {
    ...
    parentLi.classList.add("dropdown-org-parent");

    // Create toggle element
    const toggle = document.createElement("div");
    toggle.classList.add("dropdown-toggle-org");
    toggle.innerHTML = `<img src="${org.image}" alt=""> ${org.name}`;

    // Children dropdown
    const childUl = document.createElement("ul");
    childUl.classList.add("dropdown-org-children");
    ...
    if (org.children && org.children.length > 0) {
      ...
      // No children, just one link
      toggle.innerHTML = `<a href="#"><img src="${org.image}" alt=""> ${org.name}</a>`;
      parentLi.appendChild(toggle);
    }

    orgContainer.appendChild(parentLi);
  });
</script>
